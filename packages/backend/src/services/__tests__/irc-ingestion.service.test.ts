...
describe('IRC Ingestion Service', () => {
  ...
  beforeEach(async () => {
    ...
    // Create IRC profiles for profile-scoped tests
    // Note: IDs will be auto-generated by the database
    for (let i = 1; i <= 2; i++) {
      try {
        const result = await dbClient
          ...
          .values({
            integrationType: 'irc',
            name: `Test IRC Profile ${i}`,
            encryptedCredentials: 'mock_encrypted_password',
            config: JSON.stringify({ nick: `bot${i}`, server: `irc${i}.test.local` }),
          })
          .returning({ id: integrationConnectionProfiles.id });
  ...
  afterEach(async () => {
    ...
    createdProfileIds.length = 0; // Clear array for next test
  });
  ...
  describe('Profile-Scoped Conversation Mapping (INT-011)', () => {
    it('should create profile-scoped conversations with different profileIds', async () => {
      // Use the profiles created in beforeEach
      expect(createdProfileIds.length).toBe(2);
      const [profileId1, profileId2] = createdProfileIds;
    ...
    it('should reuse conversation when same profile sends to same channel', async () => {
      // Use the first profile created in beforeEach
      expect(createdProfileIds.length).toBe(2);
