...
| INT-006 | Create IRC configuration endpoint (POST /api/integrations/irc/config) | **In Review** (PR #263) | P0 | Backend | BE-005 | Saves server, port, nick, password, channels with AES-256-CBC encryption; validates all fields; RBAC super_admin only; audit logged | PVTI_lAHOAB4wV84BNGcwzgj_6MQ | 53 |
