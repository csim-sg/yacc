...
| INT-012 | Handle IRC connection errors (logging, DLQ for failed messages) | In Progress (PR #267) | P1 | Backend | INT-001, BE-015 | Connection errors logged, failed messages moved to DLQ. Contract: `traceContext.ircProfileId` is a number; callers must pass correct type (no string-to-number coercion inside DLQ service). | PVTI_lAHOAB4wV84BNGcwzgj_6NY | 64 |
