...
| SH-003 | Define WebSocket Event Types (9 events: message, conversation, notification, presence, typing) | Ready | P0 | Backend | SH-001 | TypeScript interfaces in packages/common/types/events/; one-definition-per-file (ADR-005); BaseEvent<T,D> with discriminated union; JSDoc + example payloads; 85%+ tests; zero `any` types | PVTI_lAHOAB4wV84BNGcwzgj_6hE | 308 |
| SH-004 | Create Zod Schemas for Request Validation (35+ endpoints, source of truth per ADR-020) | Ready | P0 | Backend | SH-002 | Zod schemas in packages/common/schemas/ (domain-specific files); validation middleware via middlewares option; types inferred via z.infer; user-friendly errors; 85%+ tests; ADR-020 compliance | PVTI_lAHOAB4wV84BNGcwzgj_6g8 | 309 |
| SH-005 | Set Up Shared Package Exports (domain-specific, NO single barrel export per ADR-005) | Ready | P0 | Architect | SH-001, SH-002, SH-003, SH-004 | package.json exports map configured; domain-specific aggregators (types/entities, types/api, types/events, schemas); README documented; backend + frontend imports work; tree-shaking verified; NO src/index.ts barrel | PVTI_lAHOAB4wV84BNGcwzgj_6hA | 310 |
...
| INT-010 | Support multi-profile IRC credentials in DB with DB-first gating | **Done** (PR #265 merged) | P0 | Product Owner | INT-006 | Each tenant stores up to 10 IRC profiles encrypted at rest (hard cap, no tenant-specific policy); DB-first gating (if any DB profiles exist, use only DB; env fallback only if zero DB profiles); fail-closed on DB errors (no silent env fallback); no plaintext secrets in responses/logs/audit; RBAC: Super Admin CRUD, Admin/Manager read-only; audit logging for all profile lifecycle events; deterministic E2E tests with mocked test-connection | PVTI_lAHOAB4wV84BNGcwzgj_6Mo | 61 |
| INT-011 | Map IRC channels to conversations (one conversation per channel) | **Done** (PR #267 merged) | P0 | Backend | INT-002, BE-002 | Channel joins create/update conversations, external_thread_id = channel name | PVTI_lAHOAB4wV84BNGcwzgj_6M4 | 65 |
| INT-012 | Handle IRC connection errors (logging, DLQ for failed messages) | **Done** (PR #267 merged) | P1 | Backend | INT-001, BE-015 | Connection errors logged, failed messages moved to DLQ | PVTI_lAHOAB4wV84BNGcwzgj_6NY | 64 |
| INT-013 | Write unit tests for IRC connector (connection, authentication, message handling) | **Done** (PR #267 merged) | P1 | Backend | INT-003 | 90%+ coverage for IRC connector | PVTI_lAHOAB4wV84BNGcwzgj_6M0 | 62 |
| INT-014 | Write integration tests for IRC connector (mock IRC server) | **Done** (PR #267 merged) | P1 | Backend | INT-003 | End-to-end message flow tested | PVTI_lAHOAB4wV84BNGcwzgj_6Nw | 63 |
...
| DOC-009 | Add governance log entry for PR #131 blocker fixes | Completed | P0 | Architect | DEV-131-03, DEV-131-04 | GOV-004 created with compliance checklist and Mermaid diagram | PVTI_lAHOAB4wV84BNGcwzgl42o8 | 290 |
| DOC-010 | Create WebSocket client implementation guidance (GOV-005) | Completed | P0 | Architect | DEV-131-05, DEV-131-06 | GOV-005 created with one-definition-per-file rules and observability requirements | PVTI_lAHOAB4wV84BNGcwzgl42pY | 291 |
