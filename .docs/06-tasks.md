...
| INT-010 | Support tenant-owned integration credentials in DB (integration-specific profile policy) | Not Started | P0 | Product Owner | INT-006 | Each tenant stores integration credentials encrypted at rest; integration-specific policy defines max profiles (0..1 or 0..N) and is stored in DB settings; tenant settings can only tighten (reduce maxProfiles), never loosen beyond hard limit; no plaintext secrets in responses/logs/audit; RBAC rules defined; QA scenarios defined | PVTI_lAHOAB4wV84BNGcwzgj_6Mo | 61 |
