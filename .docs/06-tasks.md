...
| INT-005 | Implement IRC connection status tracking (connected/retrying/disconnected) | **In Progress** | P0 | Backend | INT-004 | Status stored in runtime memory (no DB persistence for MVP); exposed via endpoint + WebSocket | PVTI_lAHOAB4wV84BNGcwzgj_6NE | 59 |
...
| INT-009 | Implement IRC connection status endpoint (GET /integrations/irc/status) | **In Progress** | P0 | Backend | INT-005 | Returns current connection status; RBAC admin+; no secrets exposed | PVTI_lAHOAB4wV84BNGcwzgj_6Mc | 57 |
